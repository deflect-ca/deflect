...
def generate_banjax_config(config, all_sites, formatted_time):
    ...
    all_site_password_protected_paths_exceptions = {}
    all_site_password_hashes = {}
    for _, site in client_sites.items():
        paths = site.get('password_protected_paths', [])
        exception_paths = site.get('password_protected_paths_exceptions', [])
        ...
        if len(paths) > 0:
            ...
            all_site_password_protected_paths_exceptions[site['public_domain']] = exception_paths
    ...
    banjax_next_config["password_protected_paths_exceptions"] = all_site_password_protected_paths_exceptions
